...
gameCharacters = " #@H$V*"
REV_TILES_MAP = dict(
    (s, gameCharacters[i]) for i, s in enumerate(["empty", "solid", "player", "exit", "diamond", "key", "spike"]))
print(REV_TILES_MAP)
...
def find_closest_goal_map(random_map, data_size):
    ...
    closest_map = None
    filepath = 'ddave/danger_dave_{}.txt'
...
def generate_pod_greedy(env, random_target_map, goal_starting_map, total_steps, ep_len=77, crop_size=9, render=True,
                        epsilon=0.02):
...
# Read in params:
# tile distribution for zelda_prob
...
for obs_size, episode_len, goal_set_size in obs_ep_comobs:
    ...
    while total_steps < 962500:
        save_count += 1
        play_traces = []
        cropped_wrapper = CroppedImagePCGRLWrapper("ddave-narrow-v0", obs_size,
                                                   **{'change_percentage': 1, 'trials': 1, 'verbose': True,
                                                      'cropped_size': obs_size, 'render': False})
        pcgrl_env = cropped_wrapper.pcgrl_env
        start_map = gen_random_map(rng, 11, 7, {0: 0.5, 1: 0.3, 2: 0.02, 3: 0.02, 4: 0.04, 5: 0.02, 6: 0.1})
        print(len(start_map), len(start_map[0]))
        print(start_map)

        goal_map = find_closest_goal_map(start_map, goal_set_size)
        play_trace, temp_num_steps = generate_pod_greedy(pcgrl_env, start_map, goal_map, total_steps,
                                                         ep_len=episode_len, crop_size=obs_size, render=False)
        ...
        if save_count % 250 == 0:
            ...
            df.to_csv(
                f"ddave_exp_traj_obs_{obs_size}_ep_len_77_goal_size_{goal_set_size}/pod_exp_traj_goal_step_{total_steps}.csv",
                index=False)
            exp_traj_dict = {f"col_{i}": [] for i in range(dict_len)}
            exp_traj_dict["target"] = []
