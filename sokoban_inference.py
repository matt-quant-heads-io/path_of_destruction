...
def infer(game, representation, model_path, **kwargs):
    ...
    if not non_random_start:
        agent = keras.models.load_model(f'sokoban_model_obs_{obs_size}_goal_size_{goal_map_size}_model_num_{version}.h5')
        for i in range(kwargs.get('trials', 1)):
            ...
            while not dones:
                ...
                if info[0]["solved"]:
                    ...
                    f = open(f'sokoban_playable_maps_obs_{obs_size}_ep_len_{ep_len}_goal_size_{goal_map_size}_{version}/{success_count}.txt', 'w')
                    f.write(level_str)
                    f.close()
    ...
    print(f"trial {i}, success pct is {success_pct}")
